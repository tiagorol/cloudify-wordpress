...
ctx logger info "Configurando Monitoramento Wp3..."
...
sudo cp template_private_ip_wp3.txt private_ip_wp3.txt
sudo cp template_public_ip_wp3.txt public_ip_wp3.txt

sudo sed -i "s/private_ip_wp/$HOST_PRIVATE_WP/" private_ip_wp3.txt
sudo sed -i "s/public_ip_wp/$HOST_PUBLIC_WP/" public_ip_wp3.txt
